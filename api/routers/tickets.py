...
@router.get("/api/tickets/tickpick")
def get_tickpick_ticket(
    home_team: str,
    date_time: str,
    repo: TickpickTicketQueries = Depends()
) -> TicketOut:
    ticket = repo.get_ticket(home_team, date_time)
    if not ticket:
        raise HTTPException(status_code=404, detail="Ticket not available")
    return ticket
